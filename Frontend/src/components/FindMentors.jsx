...
// 🔁 Cloned mentors array to simulate infinite scroll
const mentorsLoop = [...dummyMentors, ...dummyMentors];
    ...
    <>
      ...
      <div className='w-full bg-[#f3f3f3f7] text-black'>
        ...
        <div className='w-full flex justify-between pt-6 pb-3 px-5 lg:px-23 gap-4'>
          ...
          <button className='hidden text-sm lg:text-xl lg:flex gap-2 sm:gap-3 items-center px-4 py-1 lg:py-2 rounded-full bg-[#5BE38D] cursor-pointer hover:bg-green-400 md:px-12 justify-center'>
            <span className="font-bold text-md">VIEW ALL</span>
          ...
          <div className={`mt-4 lg:mt-0 flex lg:flex lg:justify-between lg:items-center lg:flex-row flex-col gap-4`}>
            ...
            <div className='flex gap-3 items-start sm:items-center'>
              ...
              <div className="relative w-fit flex">
                <button onClick={() => setShowSelect(prev => !prev)} className={`text-base sm:text-lg flex items-center rounded-full gap-2 font-semibold border border-black px-4 py-0 cursor-pointer overflow-hidden transition-all duration-200 ${showSelect && "w-fit"}`}>
      ...
      <div className="py-3 bg-[#f3f3f3] flex flex-col-reverse lg:flex-col">
        <div className="w-full">
          ...
          <div className="mx-10 mt-3 flex lg:hidden">
            <button className="w-full py-2 rounded-full bg-[#5BE38D] flex justify-center items-center">
              <span className="font-semibold">VIEW ALL<i className="ri-graduation-cap-line text-2xl sm:text-3xl ml-2"></i></span>
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          >
            {mentorsLoop.map((mentor, index) => {
              const selectedIndex = dummyMentors.findIndex(
                (m) => m.id === selectedMentor?.id
              );
              const isActive = index % dummyMentors.length === selectedIndex && hovered === index;
              ...
              return (
                <button
                  ...
                  className={`mentor rounded-lg overflow-hidden transition-all relative w-[150px] h-[300px] my-1 text-3xl snap-center font-semibold min-w-[100px] leading-normal flex items-center justify-center shrink-0 ripple p-1
                    ${
                      index % dummyMentors.length === selectedIndex + 1 ||
                      index % dummyMentors.length === selectedIndex - 1
                        ? "h-[350px] w-[250px]"
                        : index % dummyMentors.length === selectedIndex
                        ? "h-[400px] w-[250px]"
                        : "w-[200px] h-[300px]"
                    }
                  `}
